...
func verify_proof_rec{ range_check_ptr, bitwise_ptr : BitwiseBuiltin* }(
    ...
    if current_index == 0:
        let (local hashes_match: felt) = arr_eq(current_element_keccak, 4, root_hash.element, root_hash.element_size_words)
        assert hashes_match = 1
    else:
        let (local hashes_match: felt) = arr_eq(current_element_keccak, 4, next_hash.element, next_hash.element_size_words)
        assert hashes_match = 1
    end
    ...
    if node_len == 2:
        ...
        if current_index == proof_len - 1:
            ...
            if is_list == 0:
                ...
                let (local element_data_extracted: IntsSequence) = extractData(children.dataPosition, children.length, current_element.element, current_element.element_rlp_size_words)
                let (local next_hash_words: felt*) = keccak256{keccak_ptr=keccak_ptr}(
                    element_data_extracted.element,
                    element_data_extracted.element_size_bytes)
                local next_hash: IntsSequence = IntsSequence(next_hash_words, 4, 32)
                return verify_proof_rec(
                    path=path,
                    root_hash=root_hash,
                    proof=proof,
        ...
        if current_index == current_element.element_size_bytes - 1:
            if path_offset + 1 == path.element_size_bytes * 2:
                return extractData(node[16].dataPosition, node[16].length, current_element.element, current_element.element_rlp_size_words)
            else:
            ...
            let (revert) =  is_le(path_offset, path.element_size_bytes * 2 - 1)
            assert revert = 0
            let (local node_children: felt) = extract_nibble_from_words(path, path_offset)
            local children: RLPItem = node[node_children]
