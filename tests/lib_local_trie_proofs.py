...
def test_verify_valid_storage_proof(TestTrieProofs):
    ...
    Data.from_hex(Web3.keccak(hexstr=trie_proofs[1]['storageProof'][0]['key']).hex())
    proof_path = Data.from_hex(Web3.keccak(hexstr=trie_proofs[1]['storageProof'][0]['key']).hex())

    expected_key = Data.from_hex(str(test_trie_proofs.verify(proof.to_bytes(), account_state_root.to_bytes(), proof_path.to_bytes(), {"from": accounts[0]})))
    print(expected_key)
